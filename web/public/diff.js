...
diff.splitDiff = (diffs)=>{
	...
	diffs.forEach(diff=>{
		...
		* Если в тексте диффа есть лайн-брейк, то это конец строки.                  //Оно не работает нормально, оно может "расклеить" строку, пример "asd asd AAAA\n" --> "asd asd BBBB\n", BBBB отклеится и окажется на строке ниже.
		**/
		...
		**/
		...
		if(eotl){
			...
			if(diff.text.match(_)!==null && diff.type !== 0) //Если дифф содержит лайн-брейк и не equal
				expect_eotl = diff.type; //То мы определённо ждём появляения ещё одного не-equal диффа с лайн-брейком c противоположным типом
	});
	return n_diff;
};
